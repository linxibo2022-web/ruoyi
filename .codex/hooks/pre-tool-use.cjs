#!/usr/bin/env node
/**
 * Codex PreToolUse Hook - 工具执行前拦截
 *
 * 功能：阻止已知踩坑命令（Windows `> nul`、`rm -rf /` 等），并对敏感
 * 文件写入做提醒。匹配 Bash + apply_patch 两类工具。
 *
 * Codex stdin schema：
 ...
 *
 * Codex 输出：
 *   - {permissionDecision:"deny", message} → 阻止（推荐写法）
 *   - {decision:"block", reason} → 兼容写法
 *   - {} 或不输出 → 放行
...
if (toolName === 'Bash' || /^shell$/i.test(toolName)) {
  ...
  // 之所以做组合判定：PowerShell 路径常存在变量里（$path = 'D:\...'），
  // 单看 Remove-Item 那一行无法识别路径；只有把整段命令一起看才可靠
...
if (/^(apply_patch|Edit|Write)$/i.test(toolName)) {
  ...
  for (const f of sensitive) {
    if (filePath.endsWith(f) || patchText.includes(f)) hits.push(f);
  }
  if (hits.length) {
