...
@ActiveProfiles("dev")
@DisplayName("登录日志接口集成测试")
public class SysLoginLogIntegrationTest extends BaseControllerTest {
    ...
    public static void login(@Autowired TestLoginHelper testLoginHelper) {
        log.info("========== 登录日志测试开始,模拟登录 ==========");
        token = testLoginHelper.loginAsSuperAdmin();
    }

    @Test
    @DisplayName("测试分页查询登录日志")
    public void testPageLoginLogs() {
        log.info("测试分页查询登录日志");

        // 查询第1页,每页10条
        ForestResponse<R<PageResult<Object>>> response =
            apiClient.pageLoginLogs(token, 1, 10);
