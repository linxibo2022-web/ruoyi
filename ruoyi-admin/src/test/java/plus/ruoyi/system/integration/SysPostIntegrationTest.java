...
@ActiveProfiles("dev")
@DisplayName("岗位管理接口集成测试")
@Tag("integration")
public class SysPostIntegrationTest extends BaseControllerTest {
    ...
    public static void login(@Autowired TestLoginHelper testLoginHelper) {
        log.info("========== 岗位管理测试开始,模拟登录 ==========");
        token = testLoginHelper.loginAsSuperAdmin();
    }

    @Test
    @DisplayName("测试分页查询岗位列表")
    public void testPagePosts() {
        log.info("测试分页查询岗位列表");

        // 查询第1页,每页10条
        ForestResponse<R<PageResult<Object>>> response =
            apiClient.pagePosts(token, 1, 10);
    ...
    @Test
    @DisplayName("测试查询岗位详情")
    public void testGetPostById() {
        log.info("测试查询岗位详情");

        // 查询董事长岗位(ID=1)
        ForestResponse<R<Object>> response = apiClient.getPostById(token, 1L);
    ...
    @Test
    @DisplayName("测试获取岗位选择框列表")
    public void testGetPostOptions() {
        log.info("测试获取岗位选择框列表");

        ForestResponse<R<Object>> response = apiClient.getPostOptions(token);
