...
@ActiveProfiles("dev")
@DisplayName("在线用户接口集成测试")
public class SysUserOnlineIntegrationTest extends BaseControllerTest {
    ...
    public static void login(@Autowired TestLoginHelper testLoginHelper) {
        log.info("========== 在线用户测试开始,模拟登录 ==========");
        token = testLoginHelper.loginAsSuperAdmin();
    }

    @Test
    @DisplayName("测试分页查询在线用户")
    public void testPageOnlineUsers() {
        log.info("测试分页查询在线用户");

        // 查询第1页,每页10条
        ForestResponse<R<PageResult<Object>>> response =
            apiClient.pageOnlineUsers(token, 1, 10);
