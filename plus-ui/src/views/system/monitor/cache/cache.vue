...
import { getCacheInfo } from '@/api/system/monitor/cache/cacheApi'
import type { CacheMonitorVo } from '@/api/system/monitor/cache/cacheTypes'
import { showLoading, hideLoading } from '@/utils/modal'
const cache = ref<Partial<CacheMonitorVo>>({})
const commandstats = ref()
const usedmemory = ref()

const getList = async () => {
  showLoading('正在加载缓存监控数据，请稍候！')
  const [err, data] = await getCacheInfo()
  hideLoading()
  if (err) return
  cache.value = data
