 ...
 * Codex 为唯一基准；仅处理缺少 references/full-guide.md 或正文不一致的映射。
 */
const fs = require('fs');
...
function buildEntry(name, frontMatter, fullGuide, title = name) {
  const headings = [...fullGuide.matchAll(/^##\s+(.+)$/gm)].map(item => `- ${item[1].trim()}`).slice(0, 16);
...
for (const mapping of policy.commandMappings || []) {
  ...
  const { frontMatter, body } = splitFrontMatter(read(skill), name);
  ...
  const fullGuide = fs.existsSync(codexGuide)
    ? read(codexGuide)
  ...
  if (fs.existsSync(sharedSource) && sharedSource !== codexSkillDir) {
