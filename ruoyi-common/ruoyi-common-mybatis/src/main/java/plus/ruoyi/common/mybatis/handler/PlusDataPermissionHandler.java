...
public class PlusDataPermissionHandler {
    ...
    public Expression getSqlSegment(Expression where, String mappedStatementId, boolean isSelect) {
        try {
            ...
            try {
                currentUser = DataPermissionHelper.getVariable("user");
                if (ObjectUtil.isNull(currentUser)) {
                    currentUser = LoginHelper.getLoginUser();
                    DataPermissionHelper.setVariable("user", currentUser);
                }
            } catch (Exception e) {
                // 无法获取用户信息（可能原因：SaTokenContext未初始化、Token被冻结等）
                log.warn("获取用户身份信息失败: {}，将拒绝所有数据访问。"
                    + "如果这是单元测试或需要忽略权限的特殊场景，请使用: "
                    + "DataPermissionHelper.ignore(() -> mapper.selectList(null));", e.getMessage());
                // 返回永假条件（1=0），确保无法查到任何数据，防止数据泄露
                return denyAll(where);
            }

            // 如果用户为空，说明当前无有效的用户身份
            if (ObjectUtil.isNull(currentUser)) {
                log.warn("无法获取有效的用户身份信息，将拒绝所有数据访问。"
                    + "如果这是单元测试或需要忽略权限的特殊场景，请使用: "
                    + "DataPermissionHelper.ignore(() -> mapper.selectList(null));");
                return denyAll(where);
            }
            ...
            throw ServiceException.of("数据权限解析异常 => " + e.getMessage());
        } finally {
            try {
                DataPermissionHelper.removePermission();
            } catch (Exception ignored) {
                // removePermission 也可能失败，忽略这个异常
                log.debug("清理权限上下文时发生异常: {}", ignored.getMessage());
            }
        }
    }
    ...
    private String buildDataFilter(DataPermission dataPermission, boolean isSelect) {
        ...
        for (DataColumn dataColumn : dataPermission.value()) {
            if (dataColumn.key().length != dataColumn.value().length) {
                throw ServiceException.of("角色数据范围异常 => key与value长度不匹配");
            }
            // 包含权限标识符 这直接跳过
        ...
        for (RoleDTO role : user.getRoles()) {
            ...
            // 获取角色权限泛型
            DataScopeType type = DataScopeType.getByValue(role.getDataScope());
            if (ObjectUtil.isNull(type)) {
                throw ServiceException.of("角色数据范围异常 => " + role.getDataScope());
            }
            // 全部数据权限直接返回
     ...
     * 返回永假条件（1=0），拒绝所有数据访问
     * <p>
     * 当无法获取用户身份信息时，使用此方法作为安全兜底，
     * 确保不会因为异常被框架吞掉而导致 SQL 在无数据权限的情况下执行。
     *
     * @param where 原始 WHERE 条件
     * @return 附加了 1=0 的永假条件
     */
    private Expression denyAll(Expression where) {
        try {
            Expression denyExpression = CCJSqlParserUtil.parseExpression("1 = 0");
            if (ObjectUtil.isNotNull(where)) {
                return new AndExpression(where, denyExpression);
            }
