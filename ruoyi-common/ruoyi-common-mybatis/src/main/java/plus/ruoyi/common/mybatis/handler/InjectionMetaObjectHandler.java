...
public class InjectionMetaObjectHandler implements MetaObjectHandler {
    ...
    public void insertFill(MetaObject metaObject) {
        try {
            if (ObjectUtil.isNotNull(metaObject) && metaObject.getOriginalObject() instanceof BaseEntity baseEntity) {
                ...
                if (ObjectUtil.isNull(baseEntity.getCreateBy())) {
                    ...
                    if (ObjectUtil.isNotNull(loginUser)) {
                        ...
                        baseEntity.setCreateDept(ObjectUtils.defaultIfNull(baseEntity.getCreateDept(), loginUser.getDeptId()));
                    } else {
                        // 填充创建人、更新人和创建部门信息
                        baseEntity.setCreateBy(DEFAULT_USER_ID);
                        baseEntity.setUpdateBy(DEFAULT_USER_ID);
                        baseEntity.setCreateDept(ObjectUtils.defaultIfNull(baseEntity.getCreateDept(), DEFAULT_USER_ID));
                    }
                }
            } else {
                Date date = new Date();
            ...
            throw ServiceException.of("自动注入异常 => " + e.getMessage(), HttpStatus.HTTP_UNAUTHORIZED);
        }
    }
