<!-- core-rules-version: 2026-08-20.1 -->
...
- 以节省主线程上下文为优先：凡涉及跨多文件/目录的检索、定位、审查、分析或研究，且只读、可拆分的子任务，尽量委派给子代理执行，仅回传最终结论，避免把中间的读取、搜索与推理过程堆入主上下文。
- 相互独立的子任务尽量并行委派，不串行逐个执行。
- 委派给具体业务子代理时，以该子代理 `description` 声明的触发场景为准；用户请求命中其描述即主动调用，无需等待用户点名。
- 仅当为已知单一文件或符号的定位、一句话即可回答的简单询问时，才由主线程直接完成，不委派。
