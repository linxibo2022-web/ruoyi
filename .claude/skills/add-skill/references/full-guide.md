...
- 所有 Codex 技能名称都必须登记在 `namedSkillRouting.skills`，由校验器与 `.agents/skills/` 目录比对。仅“调用意图 + 技能名”可按名称路由；定位、解释等引用意图只有与同一技能名形成局部短语时才抑制名称路由，避免把“技能放在哪里”误判为技能调用，也不能因后文出现“说明”误伤前面的调用。
