...
> **目标**：为刚生成的模块自动创建测试文件，让 `mvn test` 和 `pnpm test:e2e` 立即可跑。
> **原则**：后端测试必生成，前端 Vitest 按需生成，E2E 配置自动追加。

##### 7.5.1 后端测试生成（必做）

**ServiceTest**（参照 `ruoyi-admin/src/test/java/plus/ruoyi/business/base/service/AdServiceTest.java`）：

```
生成文件: ruoyi-admin/src/test/java/plus/ruoyi/business/[module]/service/[Class]ServiceTest.java

要求:
- 继承 BaseServiceTest
- @Tag("dev") + @DisplayName("{功能名} Service 测试")
- @Autowired I{Class}Service
- 第一行: // @Generated by /dev — 请勿删除此行，手动修改请在标记行之后进行
- 生成前检查: 若文件已存在且含 @Generated → 跳过; 存在但无标记 → 提示用户确认
- 测试方法: testAdd / testPage / testGet / testUpdate / testDelete
- 使用 TestDataBuilder 造随机数据
- @BeforeEach 中显式设置: TenantHelper.setTenantId("000000")
```

**ControllerTest**（参照 BaseControllerTest 的 doGet/doPost/doPut/doDelete）：

```
生成文件: ruoyi-admin/src/test/java/plus/ruoyi/business/[module]/controller/[Class]ControllerTest.java

要求:
- 继承 BaseControllerTest
- @Tag("dev") + @DisplayName("{功能名} Controller 测试")
- 第一行: // @Generated by /dev — 请勿删除此行，手动修改请在标记行之后进行
- 生成前检查: 同 ServiceTest 的去重逻辑
- 测试方法覆盖六个端点: page{Class}s / list{Class}s / get{Class} / add{Class} / update{Class} / delete{Class}s
...
向 `plus-ui/src/__tests__/e2e/crud-smoke.spec.ts` 的 `CRUD_PAGES` 数组追加一项：
...
**去重**：追加前用 `apiAdd` 字段在数组中查找，已存在则跳过并提示。

##### 7.5.3 前端 Vitest 智能检测（按需）

扫描以下条件，满足任一即生成对应测试：

| 条件 | 检测方式 | 生成文件 |
|:---:|------|------|
| V1 | `plus-ui/src/composables/use{Module}.ts` 存在 | `use{Module}.test.ts` |
| V2 | `plus-ui/src/utils/{module}.ts` 存在且有 export | `{module}.test.ts` |
| V3 | `plus-ui/src/store/modules/{module}.ts` 有非通用 action | `{module}Store.test.ts` |
| V4 | `{Class}ServiceImpl.java` 方法数 > 6 | ServiceTest 追加自定义逻辑段 |
| V5 | `.vue` 文件 computed ≥ 3 | 提示"建议提取为 composable 后补测"，不自动生成 |

> 标准 CRUD 模块（V1~V4 均不满足）→ 跳过 Vitest。后续写了 composable 可通过 `/add-test 模块名` 补测。

##### 7.5.4 生成后提示

```
📋 测试生成完成：
  ✅ ServiceTest: XxxServiceTest.java
  ✅ ControllerTest: XxxControllerTest.java  
  ✅ E2E 配置: crud-smoke.spec.ts 已追加
  ⏭️ Vitest: 跳过（标准 CRUD，无自定义逻辑）

💡 验证: mvn test -Dtest=XxxServiceTest
💡 跑通后提交: git add src/test/... && git commit -m "test: 添加{功能名}测试 [scope: test]"
```
