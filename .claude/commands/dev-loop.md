...
### 子代理执行（复杂任务可选）
- 复杂 / 易上下文漂移的任务，用 **Agent 工具派一个新鲜子代理**执行单条 `docs/tasks/active/` 任务。
- 派单时把该任务的**文件清单 + 框架约束**（`plus.ruoyi` 包名 / 四层 / `TenantEntity` / `buildQueryWrapper` / `A*`·`wd-*` 组件 / API 路径）一并交给子代理，避免它丢上下文违规。
- 轻量任务（标准 CRUD 编排等）主循环直跑即可，不必派子代理。
