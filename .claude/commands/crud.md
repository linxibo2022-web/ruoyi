...
> **目标**：为刚生成的模块自动创建测试文件。与 `/dev` 的测试生成规则完全一致。
...
要求:
- 继承 BaseServiceTest, @Tag("dev"), @DisplayName
- 第一行: // @Generated by /crud — 请勿删除此行
- 生成前检查: 含 @Generated 则跳过, 无标记则提示
- 测试方法: testAdd / testPage / testGet / testUpdate / testDelete
- @BeforeEach: TenantHelper.setTenantId("000000")
```
...
要求:
- 继承 BaseControllerTest, @Tag("dev"), @DisplayName
- 第一行: // @Generated by /crud — 请勿删除此行
- 六个端点: page/list/get/add/update/delete
- 分页断言 code=200 即可（允许空数据）
```

#### 6.2 E2E 配置自动追加（必做）

向 `plus-ui/src/__tests__/e2e/crud-smoke.spec.ts` 的 `CRUD_PAGES` 数组追加一项配置。追加前用 `apiAdd` 字段去重。

#### 6.3 前端 Vitest 智能检测（按需）
...
标准 CRUD → 全部跳过。后续通过 `/add-test 模块名` 增量补测。

#### 6.4 生成后提示

```
📋 测试生成完成：
  ✅ ServiceTest / ControllerTest / E2E 配置已追加
  ⏭️ Vitest: 跳过（标准 CRUD）
